...
# 这张表是结构性防线：即使 bot 被注入说服了，**走这条路**也发不出去。
#
# 【但它不是唯一的门，别把这句话说成"发不出去"】（2026-09-08 灵犀评审指出）：
# `workflows/dev-requests` 直接 INSERT PrivateMessages 表，根本不经过本模块，
# `allowed()` 一次都不会被调用。（`chengyu-upload` 曾是第二处，
# 2026-09-08 已随听书模块清空而退役，见 examples/retired-audiobook-lines/。）
#
# 现状风险有限——它的收件人由业务数据固定（需求单的 UserId，取自 JWT 而非请求体，
# 见 DevRequestController.cs:71-73），不是内容里能指定的，
# 所以"注入让它发给任意人"这条路走不通。
# 但**防线只覆盖走这扇门的调用方**这件事必须写明：
# 说成"结构性地发不出去"是把覆盖面吹大了，而吹大的安全声明比没有更危险。
# 记为 docs/specs/002-known-gaps.md 缺口十四。
OUTBOUND_WHITELIST = {}
