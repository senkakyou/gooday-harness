...
### G20 Markdown 排版统一 `[可检查]`

文档是**给人读的主要界面**——新人接手先读 README，出事先读 runbook。
排版不统一的代价不是"不好看"：标题前不空行会被某些渲染器吞掉标题，
列表前不空行会被当成上一段续行整段挤在一起，首行不是 H1 会让目录生成器抓不到标题。

> **在终端里勉强能看，在渲染后就散了。**

规矩取最小集，每条都能自动判定，不做风格偏好之争：
首行 H1、标题/列表/表格/代码块前留空行、无行尾空格、恰好一个结尾换行、标题不跳级。

`prompt.md` 豁免首行规则——它不是文档，是喂给模型的数据，
首行的 HTML 注释由 `runner.py` 剥掉再送模型，强改会改变模型读到的内容。

自动修：`python3 evolution/gates/rules/G/g20_markdown_format.py --fix`
（只加空行、去行尾空格，**不改任何字**；58 个文件上验证过内容零变动）

### G21 客户端与服务端限值对齐 `[可检查]`

**服务端拒绝的条件，客户端必须知道。** 不知道的后果不是报错，是内容整条消失。

`packages/botkit/outbound.py` 的 `MAX_CONTENT` 必须与
`PrivateMessageController` 里 `content.Length > N` 的 N 相等。
**并且每一个直发 `/api/messages/` 的文件都要认识这个上限**——
要么自己声明，要么复用 botkit 的（检查器扫 `packages/` `workflows/` `services/`）。

配套两条：

- **4xx 一律不重试**（401 除外，那是能自愈的）。请求本身坏掉时，
  重试三次就是三次必然失败，而日志里三行一样的报错会被当成网络抖动。
- **保证放在 `send()` 里，不放在调用方**。超长时它自己分段，
  调用方不需要知道上限——**正确的用法必须是最省事的用法**。
- **单段失败不 break，最后必须补一条「第 x/y 段失败」**。
  不补的话收件人看到的是一段完整的话，完全不知道后面还有——
  比明说「缺了几段」危险得多。走不了 `send()` 的调用方用
  `outbound.send_each(content, send_one)`，别自己搓循环。

> 教训：2026-09-08 灵犀写了一份长评审回给站长，超过 4000 字上限被判 400，
> `send` 当成可重试错误试了三次——**整份评审消失，日志里真因一个字都没有**。
> 更刺眼的是 `send_segments`（分段发送）当时**就在同一个文件里**，
> 只是没有任何人调用它：又一个「能力写好了但没接上」，和
> 「注入条款写进文档三个月没进 prompt」同形。
>
> 第一版规则只盯 `outbound.py` 一个文件就报绿了，**而同一个 bug 在
> daily-report / finance-report / coo-patrol 三处原样活着**——
> 它们各自手搓了发送函数，产的还都是长文本（日报、财报、巡检报告）。
> coo-patrol 最讽刺：它 import 了 outbound 只借 `allowed()` 做白名单，
> 发送走自己的 —— **只借了半个能力，另一半的坑照样踩。**
> **窟窿不在调用图，在作用域**（2026-09-08 灵犀评审指出）。
>
> **单位也要对齐，不只是数字。** 服务端 `string.Length` 数的是 UTF-16 码元，
> Python `len()` 数的是码点，BMP 外字符（emoji）差一倍。
> 一度写成「靠分段预留的 16 字余量兜着」——**那个推理是错的**：
> 误差跟段内 astral 字符**线性相关**，一段里 17 个 🔴 就破，
> 而日报的优先级标恰好在用 🔴🟠⚪🔵。现已统一走 `outbound.ulen()`。
>
> **作用域按端点分，不按语言分。** 前端也在管辖内：
> `Messages.jsx` 走 REST（上限 4000），`ChatBox.jsx` 走 SignalR
> （`ChatHub.cs` 自己判 500，它的 `maxLength={500}` 是对齐的）。
> 客户端比服务端**松**是 ERROR（打完了才发现发不出去）；
> 比服务端**严**是 WARN（不丢消息，但用户打到上限静默打不动，没提示没计数）。

### 元规则：检查失败 ≠ 检查通过

任何规则在【无法完成检查】时，必须报 ERROR，不能报 SKIP。
**SKIP 只留给「这里本来就没有可查的东西」**，绝不能用来表示「查不成」。

以 git 为例，四种状态必须严格区分，塌缩成布尔值就会出错：

| 状态 | 含义 | 判定 |
|---|---|---|
| `ok` | 正常 | 继续检查 |
| `no_repo` | 不是 git 仓库 | **SKIP**——这里本来就没版本库 |
| `no_git` | git 命令不存在 | **ERROR**——检查没跑成 |
| `failed` | 执行失败（dubious ownership / 权限 / 超时） | **ERROR**——本该跑却没跑成 |

> 教训：原实现只有 True/False 两态，后三种全塌缩成「git 不可用」。
> 更早的版本更糟——把任何 git 失败都吞成空字符串，于是以 root 跑时
> （dubious ownership）门禁打印「不是 git 仓库，跳过」然后**放行**，
> 而部署入口正是 `sudo bash install.sh`，恰恰是最需要它的时刻。
>
> **「检查失败却 SKIP 放行」是本项目最痛恨的形态**，检查器自己更不能犯。
...
未接入的，本仓库点过名的是 **C01、C09、C12、C17、C19**
（写成「C01–C12」是不诚实的：那个区间包进了本仓库从没提过的编号，
又把 C17、C19 漏在外面。`ai-company-kit` 的完整清单不在本仓库，**没见过的别替它编号**）。

**当初不接的理由已经不成立了**——那时写的是「`services/` 还没有成员，
接进来只会是一堆永远 SKIP 的规则」，现在 `services/` 有 9 个成员、5 个是 bot，
它们正是 C 层要管的东西。

两条尤其刺眼：

- **C17**（常驻服务改完确认真重启）在 `docs/specs/005-policy-layers.md` 里被列进
  「现在就接入」的 5 条，**至今没有实现，也没登记进本表**。
  它躲过 G05 是因为 G05 查的是「本表 ↔ `gates/rules/`」，
  而它连本表都没进 —— **spec 说要接、总表没登记，这段落差没有任何检查器在看。**
  （运行期行为其实有：`workflows/patrol/checks/stale_process.py` 在查同一件事。）
- **C19**（事故转规范）是 H11 被吸收的目的地。被指过去，然后从待办清单里掉了出去。

这两条记在 `docs/specs/002-known-gaps.md` 缺口九，**别再拿旧理由搪塞**。

### C18 prompt 注入防御 `[可检查]`

有 `prompt.md` 的服务必须含注入防御条款；且 prompt 里**不得写死敏感路径**。

> 教训：2026-09-07 迁移时逐个查过——**五个 bot 的防御条款全部是 0 处**。
> 防御文档在 `docs/` 里躺了三个月，一个 bot 都没带上。
>
> 最危险的是灵犀：它有 Bash 与文件读写，**注入成功 = 攻击者拿到 shell**。
> 而它的 prompt 里还硬编码了 **16 处**敏感路径（数据库绝对位置、`.env` 在哪、
> `JWT_SECRET` 在哪）——等于把攻击面写进它自己的提示词。
>
> 判据用哨兵串「安全与边界」，**不能用裸的「注入」二字**：
> 代码里的「注入上下文」是完全无关的意思，用它当判据会让毫无防御的
> prompt 全部误判通过（本项目第一版就是这么错的）。
