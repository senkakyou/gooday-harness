...
12 个收敛为 5 个：`NEW → IN_PROGRESS → DELIVERED → CLOSED`，加 `BLOCKED` / `CANCELLED`。
...
`Tickets` 清空并瘦身 23 列 → 13 列。新建 `TicketLog`（工作记录）。
...
1. **去掉了一道结构性收款保护。** 原来 `CONFIRMED → IN_PROGRESS` 与 `→ done` 都硬查
   ...
   **缓解**：闸⑤保留「金额非空才让结单」的代码兜底，否则财务表对不上而账面看着是好的。
   **这一条是大海明确要的，不是我顺手删的。**

2. **删掉自动流转 = 没人推的单会一直躺着。** 原来 coo-patrol 有卡死看门狗。
   **缓解**：patrol 加一条「`NEW` 超 3 天没动 → 提醒大海」。
