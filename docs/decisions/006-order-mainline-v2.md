...
- **判定**：通过
- **上线时间**：2026-09-10（分八个提交灰度落地，每步可独立回滚）
- **合并 main**：2026-09-11，快进到 `a65f95e`（= 灵犀复核过的那个 hash，无额外合并提交）

### 灵犀三轮对抗性评审（她拍板是否可合，大海授权）

| 轮次 | 结论 | 她抓到的 |
|---|---|---|
| 一 | **改完再合** | ① 我边审边改副本，她读的不是声称的那个 hash ② **锁了动作没锁判据字段**（PUT 能改 ClientId/Amount，绕过 release/close 闸）③ 闸⑤的理由是假话（Amount 与 FinanceRecords 全仓零核对）④ install.sh 的 `rm -rf state/$name` 会误伤同名 workflow ⑤ intake 防灌按模型吐的 contact 计数，换串即绕过 |
| 二 | **还要改两条** | ① `ClientId` 为 null 时**整条归属校验不执行**，而匿名表单建的单恒为 null（主拉新入口）② `Clients.UserId` 是判据沿路读到的字段，却没锁，绑给攻击者后闸门翻向且**全程零信号** |
| 三 | **合 main** | 额外自查了第三个入口（`PUT /api/clients/{id}` 的 DTO 无 UserId 字段，无绕路）；并指出**第五次换皮**（`Tools.OwnerUserId`/`SourceTicketId` 只认 role=admin）判为不阻塞，已记入 known-gaps 缺口十九 |

**同一个错误换了五次皮**，这是本次重构最值钱的一条教训，规则见 known-gaps 缺口十九。

### 真单实跑（灵犀要求「别让生产是首跑」）

用测试账号走完 建单→开工→build→灵犀打回→钉死口径重做→灵犀通过→上架（**客户零通知，
实测命中 0**）→大海放行→如意通知客户（实测 1 条，发信人确为如意）→CLOSED，产物已清理。

**第一趟崩了四次，四个都是真缺陷**：order 的 README 用法用实际身份跑不起来；
install.sh 的 chown 在建目录之前导致新成员 state 目录全是 root 属主；
我为前者引入的快照被消息轮询当成实时数据源（**灵犀 70 秒就回了，而 review
等满 20 分钟报「还没回」，症状和她真的慢一模一样**）；`sudo sqlite3` 参数顺序。

### 我在本次写出过四个「绿着但没测到被测对象」的测试

都已修正并留痕，记在这里因为这是我的惯性错误：

1. intake 自测两条被**长度闸抢先拦下**，被测的白名单/违规词规则一次没执行
2. e2e 第 9 节标题写「结单闸：金额非空」，断言的其实是另一条
3. 检查器把 **405 当成「路由存在」**，给一个还不存在的路由发了通过
4. e2e 7.1 拿**连交付物都没有**的单测 ClientId 判据，在前一条就被拦住
