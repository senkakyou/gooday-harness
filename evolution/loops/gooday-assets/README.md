...
| ① | 通过率必须**超过**基线 | 持平也不算改进 |
| ② | 已正常的一项都不许变坏 | 回归 |
| ③ | **可达条目的绝对数**不许减少 | 靠缩分母提分 |
| ④ | 必须有实质进展（可达变多，或缺失变少且可达不减） | 什么都没干 |
| ⑤ | 不许某工具新指向**别人的**文件 | 「安静的错误」——分数涨、用户下到别人的内容 |

③④ 是评审时发现原判据是恒真式后重写的；⑤ 是对 Gate 做攻击测试打出来的洞。
**⑤ 尤其重要**：Learn 那边的 taken 判据能挡住*生成*这种候选，
但 improve 迟早换成模型驱动的 —— 到那时 Gate 是唯一一道线。

## 规模

实测（合成数据，单轮各阶段耗时）：

| Tools 行数 | run | all_refs | checkpoint | variant | 合计 |
|---|---|---|---|---|---|
| 1,000 | 0.06s | 0.15s | 0.01s | 0.07s | ~0.3s |
| 20,000 | 0.54s | 2.1s | 0.10s | 0.54s | ~3s |
| 100,000 | 3.7s | 9.5s | 0.56s | 2.9s | ~16s |

瓶颈是逐条 `os.path.realpath` 的系统调用，不是查询。日跑一次的任务
到十万行仍可接受。**当前真实表 1018 行，可忽略** —— 现在优化是过早优化。
真到需要时的方向：realpath 结果按目录缓存。
